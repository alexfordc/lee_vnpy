...
class Strategy_TripleMa_v2(CtaFutureTemplate):
    """15分钟级别、三均线策略
    策略：
    ...
    def export_klines(self):
        ...
        self.kline_x.export_fields = [
            ...
            {'name': f'atr{self.kline_x.para_atr1_len}', 'source': 'line_bar', 'attr': 'line_atr1', 'type_': 'list'},
        ]
    ...
    def on_bar_x(self, bar: BarData):
        ...
        if self.kline_x.ma12_count >= 1 and sub_tns_count <= 0:
            ...
            self.policy.sub_tns = {'count': self.kline_x.ma12_count, 'price': self.cur_price}
    ...
    def tns_update_price(self):
        ...
        # 持有多仓/空仓时，更新最高价和最低价
        if self.position.pos > 0:
            self.policy.tns_high_price = max(self.cur_price, self.kline_x.line_bar[-1].high_price,
                                             self.policy.tns_high_price)
        if self.position.pos < 0:
            if self.policy.tns_low_price == 0:
                self.policy.tns_low_price = self.cur_price
            else:
                self.policy.tns_low_price = min(self.cur_price, self.kline_x.line_bar[-1].low_price,
    ...
    def tns_logic(self):
        ...
        if self.kline_x.line_ma3[-1] > self.kline_x.line_ma3[-2] \
                and self.cur_price > self.kline_x.line_ma3[-1]:
    ...
    def tns_open_logic(self):
        ...
        if self.policy.tns_direction == Direction.SHORT \
            ...
            if self.tns_short():
                ...
                self.policy.last_under_open_price = self.cur_price
                # 更新最低价
                self.policy.low_price_in_short = self.cur_price
                # 设置前高为止损价
    ...
    def tns_get_volume(self, stop_price: float = 0, invest_percent: float = None):
        ...
        # 投资资金总额允许的开仓数量
        max_unit = round(float(invest_money / (self.cur_price * symbol_margin_rate)), 2)

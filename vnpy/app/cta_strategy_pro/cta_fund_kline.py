...
from vnpy.app.cta_strategy_pro.cta_line_bar import CtaHourBar
...
class FundKline(object):
    def __init__(self, cta_engine, setting, use_cache=False, load_trade=False):
        ...
        self.profit_list = []
        self.holding_list = []
        self.minDiff = self.setting.get('minDiff', 0.01)
        self.symbol = self.setting.get('symbol', None)
        if self.symbol is None:
            self.symbol = self.setting.get('vtSymbol', 'fund')
    ...
    def load(self):
        ...
        :return:
        """
        self.kline_file = os.path.abspath(os.path.join(get_folder_path('data'), 'fund_{}.csv'.format(self.kline_name)))
    ...
    def load_trade_list(self):
        ...
        :return:
        """
        trade_csv_file = os.path.abspath(os.path.join(get_folder_path('data'), '{}_trade.csv'.format(self.kline_name)))
        ...
        for _, row in trade_df.iterrows():
            try:
                ...
                vtSymbol = row.get('vtSymbol', None)
                volume = row.get('volume', None)
                trade_time = row.get('tradeTime', None)
                ...
                if direction == u'多' and offset == u'开仓':
                    exist_buy_list = self.long_pos_dict.get(vtSymbol, [])
                    exist_buy_list.append({'volume': volume, 'price': price, 'open_time': trade_time})
                    self.long_pos_dict.update({vtSymbol: exist_buy_list})
                    continue

                if direction == u'空' and offset == u'开仓':
                    exist_short_list = self.short_pos_dict.get(vtSymbol, [])
                    exist_short_list.append({'volume': volume, 'price': price, 'open_time': trade_time})
                    self.short_pos_dict.update({vtSymbol: exist_short_list})
                    continue
                    ...
                    while (sell_volume > 0):
                        if len(exist_buy_list) == 0:
                            self.write_log(u'{}没有足够的{}多单，数据不齐全,需要补全.{}'
                                           .format(self.kline_name, vtSymbol, row), strategy_name=self.kline_name)
                            break
    ...
    def get_hold_pnl(self, log=False, update_list=False):
        ...
        for vt_symbol, long_trade_list in self.long_pos_dict.items():
            cur_price = self.cta_engine.qry_price(vt_symbol)
            if cur_price is None:
                continue
            cur_size = self.cta_engine.qrySize(vt_symbol)
    ...
    def update_trade(self, trade):
        ...
        try:
            ...
            if trade.direction == Direction.LONG and trade.offset == Offset.OPEN:
                exist_buy_list = self.long_pos_dict.get(trade.vtSymbol, [])
                exist_buy_list.append({'volume': trade.volume, 'price': trade.price, 'open_time': trade.tradeTime})
                self.long_pos_dict.update({trade.vtSymbol: exist_buy_list})
                self.write_log(u'更新{}的持仓记录:{}'.format(trade.vtSymbol, exist_buy_list))
                return

            if trade.direction == Direction.SHORT and trade.offset == Offset.OPEN:
                exist_short_list = self.short_pos_dict.get(trade.vtSymbol, [])
                exist_short_list.append({'volume': trade.volume, 'price': trade.price, 'open_time': trade.tradeTime})
                self.short_pos_dict.update({trade.vtSymbol: exist_short_list})
                self.write_log(u'更新{}的持仓记录:{}'.format(trade.vtSymbol, exist_short_list))
                return
            ...
            if trade.direction == Direction.SHORT and trade.offset in [Offset.CLOSE, Offset.CLOSETODAY,
                ...
                while (sell_volume > 0):
                    if len(exist_buy_list) == 0:
                        self.write_error(
                            u'{}没有足够的{}多单记录，数据不齐全.{}'.format(self.kline_name, trade.vtSymbol, trade.__dict__))
                        return
                    buy_trade = exist_buy_list.pop(0)
                    buy_volume = buy_trade.get('volume', 0)
                    trade_volume = 0
                    ...
                    symbol_size = self.cta_engine.qrySize(trade.vtSymbol)
                    ...
                    profit_record = {'open_time': buy_trade.get('tradeTime'),
                                     'vtSymbol': trade.vtSymbol,
                                     'open_action': 'Buy',
                                     'volume': trade_volume,
                                     'open_price': buy_trade.get('price', 0.0),
                                     'close_time': trade.tradeTime,
                ...
                if len(trade.tradeTime) > 8 and ' ' in trade.tradeTime:
                    try:
                        trade_dt = datetime.strptime(trade.tradeTime, '%Y-%m-%d %H:%M:%S')
                    except Exception:
                        trade_dt = datetime.now()
                ...
                self.write_log(u'{} 多单 {} => {}平仓，当前平仓收益:{}，策略收益:{}'
                               .format(self.kline_name, trade.vtSymbol, trade.tradeTime,
            ...
            if trade.direction == Direction.LONG and trade.offset in [Offset.CLOSE, Offset.CLOSETODAY,
                ...
                while (cover_volume > 0):
                    if len(exist_short_list) == 0:
                        self.write_error(u'{}没有足够的{}空单，数据不齐全,数据需要补全.{}'
                                         .format(self.kline_name, trade.vtSymbol, trade.__dict__))
                        return
                    short_trade = exist_short_list.pop(0)
                    short_volume = short_trade.get('volume', 0)
                    open_time = short_trade.get('open_time', None)
                    ...
                    close_profit += cur_profit
                    self.closed_profit += cur_profit
                    profit_record = {'open_time': short_trade.get('tradeTime'),
                                     'vtSymbol': trade.vtSymbol,
                                     'open_action': 'Short',
                                     'volume': trade_volume,
                ...
                cur_openIntesting = self.kline.line_bar[-1].open_interest if len(
                    self.kline.line_bar) > 0 else close_profit
                self.write_log(f'{self.kline_name} {open_time} {trade.vtSymbol}空单 => '
                               f'{trade.tradeTime} 平仓，累计平仓收益:{close_profit}，策略收益:{cur_openIntesting}')
                trade_dt = datetime.now()
                if len(trade.tradeTime) > 8 and ' ' in trade.tradeTime:
                    try:
                        trade_dt = datetime.strptime(trade.tradeTime, '%Y-%m-%d %H:%M:%S')

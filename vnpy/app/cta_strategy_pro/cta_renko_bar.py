...
class CtaRenkoBar(object):
    ...
    def add_bar(self, bar, *args, **kwargs):
        ...
        if len(self.open_array) > self.max_hold_bars:
            del self.open_array[0]
        self.open_array.append(bar.open_price)

        if len(self.high_array) > self.max_hold_bars:
            del self.high_array[0]
        self.high_array.append(bar.high_price)

        if len(self.low_array) > self.max_hold_bars:
            del self.low_array[0]
        self.low_array.append(bar.low_price)

        if len(self.close_array) > self.max_hold_bars:
            del self.close_array[0]
...
    ·在10周期内当价格方向不明确的时候，自适应均线应该是横向移动；
        ...
        if self.cur_ama_pre_diff != 0:
            diffRate = round((1 + self.cur_ama_avg_diff / self.cur_ama_pre_diff) * 50, 2)
        else:
            diffRate = 0
        ...
        if len(self.line_ama) > 2:
            ...
            mtmRate = round(mtmDiff / (self.height * self.price_tick), 2)
    ...
    def __count_boll(self):
        ...
        if self.para_boll2_len > 0:
            if self.bar_len < min(14, self.para_boll2_len) + 1:
                ...
                # 1标准差
                std = (upper_list[-1] - lower_list[-1]) / (self.inputBoll2StdRate * 2)
    ...
    def rt_count_boll(self):
        ...
        if self.para_boll_len > 0:
            ...
            if self.inputBollTBLen == 0:
                ...
                # 1标准差
                std = np.std(np.append(self.close_array[-self.para_boll_len:], [self.line_bar[-1].close]), ddof=1)
                middle = np.mean(np.append(self.close_array[-self.para_boll_len:], [self.line_bar[-1].close]))
                self._rt_middle = round(middle, self.round_n)
                self._rt_upper = round(middle + self.para_boll_std_rate * std, self.round_n)
                self._rt_lower = round(middle - self.para_boll_std_rate * std, self.round_n)
        ...
        if self.para_boll2_len > 0:
            ...
            if self.inputBoll2TBLen == 0:
                ...
                # 1标准差
                std = np.std(np.append(self.close_array[-self.para_boll2_len:], [self.line_bar[-1].close]), ddof=1)
                middle = np.mean(np.append(self.close_array[-self.para_boll2_len:], [self.line_bar[-1].close]))
                self._rt_middle2 = round(middle, self.round_n)
                self._rt_upper2 = round(middle + self.para_boll_std_rate * std, self.round_n)
                self._rt_lower2 = round(middle - self.para_boll_std_rate * std, self.round_n)
...
 
    ...
    def __count_cci(self):
        ...
        # 计算第1根CCI曲线
        barCci = ta.CCI(high=np.array(self.high_array[-self.para_cci_len:], dtype=float),
    ...
    def __count_skd(self):
        ...
        if self.cur_skd_count <= 0:
            if golden_cross:
                ...
                self.skd_last_cross = (self.line_sk[-1] + self.line_sk[-2] + self.line_sd[-1] + self.line_sd[-2]) / 4
                self.skd_rt_count = self.cur_skd_count
                ...
                self.write_log(
                    u'{} Skd Gold Cross:{} at {},{}'.format(self.name, self.skd_last_cross, self.skd_cross_price,
                                                            self.cur_datetime))
            ...
            if dead_cross:
                ...
                self.write_log(
                    u'{} Skd Dead Cross:{} at {},{}'.format(self.name, self.skd_last_cross, self.skd_cross_price,
                                                            self.cur_datetime))
    ...
    def is_skd_high_dead_cross(self, runtime=False, high_skd=None):
        ...
        # 非实时，高位死叉
        if self.cur_skd_count < 0 and self.skd_last_cross > high_skd:
            return True
    ...
    def is_skd_low_golden_cross(self, runtime=False, low_skd=None):
        ...
        # 非实时低位金叉
        if self.cur_skd_count > 0 and self.skd_last_cross < low_skd:
            return True
    ...
    def get_data(self):
        ...
        return {
            ...
            'symbol': self.line_bar[-1].vtSymbol,
